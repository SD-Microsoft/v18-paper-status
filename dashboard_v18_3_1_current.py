...
# =========================
V16_ENGINE_VERSION = "V18.3.1-NEWSINTEL-1.31"
...
def _news_bundle(targets):
    ...
    for key,query in targets.items():
        ...
        result["targets"][key]=_news_analyze(items, query=query, target_key=key)
    try:
...
def run_backtest(price_df, fee_rate=COMMISSION_RATE, min_fee=MIN_COMMISSION,
    ...
    alloc_fraction = max(0.0, min(float(max_alloc_pct), 100.0)) / 100.0
...
def v18_stress_test(price_df, strategy_name, initial_cash, fx_rate, stop_pct, take_profit_pct, max_alloc_pct=20.0):
...
def candidate_parameter_search(price_df, capital_nok, fx, slippage_bps=5.0, max_alloc_pct=20.0):
...
def robust_walk_forward(price_df, capital_nok, fx, slippage_bps=5.0, folds=3, max_alloc_pct=20.0):
    ...
    for fold in range(folds):
        ...
        if len(train) < 80 or len(test) < 55:
            continue
        search = candidate_parameter_search(train, capital_nok, fx, slippage_bps, max_alloc_pct)
        if search.empty:
            continue
        best = search.iloc[0]
...
def validate_candidate(price_df, currency, capital_nok, slippage_bps=5.0, stop_pct=7.0, target_pct=14.0, max_alloc_pct=20.0):
    fx = fx_to_nok(currency)
    ...
    rwf = robust_walk_forward(df, capital_nok, fx, slippage_bps, folds=4, max_alloc_pct=max_alloc_pct)
    reasons = []
    passed = True
    ...
    # V18.1: stress the held-out region rather than reusing the full parameter-selection sample.
    stress = v18_stress_test(test, name, capital_nok, fx, chosen_stop, chosen_target, max_alloc_pct)
...
st.write("### V18.3 Failure Envelope / Capital Reality Check")
...
st.caption("V18.3 treats stop-loss, take-profit, costs, slippage, held-out data and walk-forward results as part of the strategy—not optional extras.")
...
st.write("### V18.3.1 News & Event Intelligence — RESEARCH ONLY")
st.caption("Fresh headline metadata from global news plus best-effort public forums. V18.3.1 rejects stale/off-topic matches before scoring, deduplicates evidence, separates event categories and heavily discounts forum evidence. News cannot override failed quantitative validation.")
news_table=[]
for key,info in news_intel.get("targets",{}).items():
    news_table.append({"Target":key,"News score":info.get("score",0.0),
...
auto_execute = st.toggle(
    ...
    help="Simulation only. When enabled, eligible V18.3 BUY/EXIT actions are written to the local paper-trade CSV. No Nordnet order endpoint is called.",
)
...
try:
    status_payload = {
        ...
        "news_intelligence": news_intel if "news_intel" in locals() else None,
        "news_policy": {
            "mode": "RESEARCH_ONLY",
            ...
            "relevance_filter": "V18.3.1 strict",
        },
        "rejection_summary": {
...
with st.expander("Test automation / monitoring heartbeat"):
    ...
    auto_refresh = st.toggle("Auto-refresh dashboard every 30 seconds", value=True, key="test_auto_refresh")
    if auto_refresh:
...
st.caption("V18.3.1 NEWS + QUANT ENGINE • 2,000 NOK PAPER LAB • NO BROKERAGE ORDER ENDPOINTS • Signals are research outputs, not guaranteed forecasts.")
